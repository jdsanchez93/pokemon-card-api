...
```bash
pokemon-card-api$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
pokemon-card-api$ sam local start-api
pokemon-card-api$ curl http://localhost:3000/
```
...
```bash
pokemon-card-api$ sam logs -n NetCodeWebAPIServerless --stack-name pokemon-card-api --tail
```
...
```bash
pokemon-card-api$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name pokemon-card-api
```
